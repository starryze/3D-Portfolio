...
const Computers = ({ isMobile }) => {
  ...
  useFrame(() => {
    if (ref.current) {
      const rotationY = ref.current.rotation.y;
      const radius = 10; // Distance from the center of the model
      const x = radius * Math.sin(rotationY);
      const z = radius * Math.cos(rotationY);
      pointLightRef.current.position.set(x, 10, z);
    }
  });
